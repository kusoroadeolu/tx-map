...
public class ContentionBenchmark {
    ...
    // -------------------------------------------------------------------------
    // Write Heavy Plus Size — 90% put, 10% get
    // -------------------------------------------------------------------------

    @Benchmark
    @Threads(1)
    public void writeHeavy_plusSize_1thread(ThreadState ts, Blackhole bh) {
        writeHeavyPlusSize(ts, bh);
    }

    @Benchmark
    @Threads(2)
    public void writeHeavy_plusSize_2threads(ThreadState ts, Blackhole bh) {
        writeHeavyPlusSize(ts, bh);
    }

    @Benchmark
    @Threads(4)
    public void writeHeavy_plusSize_4threads(ThreadState ts, Blackhole bh) {
        writeHeavyPlusSize(ts, bh);
    }

    @Benchmark
    @Threads(8)
    public void writeHeavy_plusSize_8threads(ThreadState ts, Blackhole bh) {
        writeHeavyPlusSize(ts, bh);
    }
    ...
    private void readHeavyPlusSize(ThreadState ts, Blackhole bh) {
        boolean isWrite = (ts.opIndex++ % 10) == 0; // 1 in 10 ops is a write
        doOpPlusSize(ts.nextKey(), isWrite, bh);
    }

    private void balancedPlusSize(ThreadState ts, Blackhole bh) {
        boolean isWrite = (ts.opIndex++ % 2) == 0; // every other op is a write
        doOpPlusSize(ts.nextKey(), isWrite, bh);
    }

    private void writeHeavyPlusSize(ThreadState ts, Blackhole bh) {
        boolean isWrite = (ts.opIndex++ % 10) != 0; // 9 in 10 ops is a write
        doOpPlusSize(ts.nextKey(), isWrite, bh);
    }

    private void doOpPlusSize(String key, boolean isWrite, Blackhole bh) {
        try (var tx = txMap.beginTx()) {
            if (isWrite) {
