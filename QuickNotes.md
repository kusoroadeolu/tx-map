...
#### Ops per transaction (Measured at 4 threads)
At one operation per transaction, the thrpt for the unbound combiner surpasses other combiners by ~50% - ~98%  however as ops per tx increases to 10, the unbound combiner's thrpt drops to ~1.1M ops/s while the sync combiner drops to ~900K ops/s around a ~22% gap between both combiners, with the array combiner having the lowest thrpt overall at ~840k ops/s and the node cycling combiner having the second highest at ~1M ops/s
This is due to batching and handoff mechanisms of the combiners amortizes(reduces) the combining overhead over multiple operations while the sync combiner has to pay the cost of obtaining a lock per transaction 

#### Thrpt as num of transactions increases(Measured with 2 ops per tx)
At one thread(under low contention), sync combiner shows the highest thrpt at ~9.7M ops/s(though with a high variance of 1M ops/s) with array showing the lowest at 3.3M ops/s. Unbound and Node cycling combiner both show ~7.6M ops/s and ~6.6M ops/s respectively under low contention.
At 8 threads(under high contention), sync combiner shows the highest thrpt at 5.3M ops/s with the array combiner showing the lowest thrpt overall at 2.1M ops/s. Both the Node cycling and unbound combiner degrade gracefully to ~3.5M ops/s and ~3.9M ops/s respectively;
Worth noting that a simple lock under high contention benefits from the JVM's biased locking optimizations, whereas the combining overhead of the other combiners doesn't get those same benefits.
